...
function App() {
  ...
  // Fetch all room content from the backend (generated by scenario_generator.py).
  // Falls back to whatever data.js already loaded if any fetch fails.
  React.useEffect(() => {
    const roomIds = [1, 2, 3, 4, 5, 6];
    Promise.all(
      roomIds.map((id) =>
        window.Api.getRoomContent(id)
          .then((data) => ({ key: 'r' + id, data }))
          .catch(() => null)
      )
    ).then((results) => {
      results.forEach((r) => {
        if (r && r.data) window.QuestData.CONTENT[r.key] = r.data;
      });
  ...
  const openRoom = (room) => {
    if (!window.QuestData.CONTENT[room.id]) return;
    sfx('enter');
    setActiveRoom(room);
